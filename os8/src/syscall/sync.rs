...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
    let detect_on = process_inner.deadlock_on;

    if detect_on && process_inner.available[mutex_id] == 0 {
        drop(process_inner);
        drop(process);
        return -0xDEAD;
    } else {
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    let sem = Arc::clone(process_inner.semaphore_list[sem_id].as_ref().unwrap());
    let detect_on = process_inner.deadlock_on;
    let need = sem.inner.exclusive_access().wait_queue.len();
    if detect_on && process_inner.available[sem_id] - 1 <= need as i8 {
        drop(process_inner);
        return -0xDEAD;
    } else {
        process_inner.available[sem_id] -= 1;
...
pub fn sys_enable_deadlock_detect(_enabled: usize) -> isize {
    match _enabled {
        1 => {
            let process = current_process();
            let mut process_inner = process.inner_exclusive_access();
            process_inner.deadlock_on = true;
        ...
        0 => {
            let process = current_process();
            let mut process_inner = process.inner_exclusive_access();
            process_inner.deadlock_on = false;
